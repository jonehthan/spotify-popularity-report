...
def build_best_model(input_dim, optimizer):
    model = Sequential([
...
for name, opt in optimizers.items():
    ...
    model = build_best_model(X_train.shape[1], opt)
    ...
    plt.title(f"{name} Optimizer – Loss Curve")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)

    filename = f"{name}_optimizer_loss.png"
